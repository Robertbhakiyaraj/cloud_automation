...
module "kv_des_cus" {
  ...
  pe_subnet_id                    = var.subnet_map["db"].subnet_details.subnet_id   #data.azurerm_subnet.kv_subnet.id
  ...
  network_acls = {
    ...
    virtual_network_subnet_ids =  [var.subnet_map["db"].subnet_details.subnet_id]
  }
...
resource "azurerm_key_vault_key" "des_key_db" {
