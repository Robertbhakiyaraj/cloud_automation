module "create_linux_vms" {
  ...
  for n in range(1, 2) : "nic${n}" => {
      name                                   = "${var.hostname_prefix}-${var.app_name}-${var.env_name}-nic-${each.key}"
      ...
      nic_ip_configurations = [
        {
          name                               = "${var.hostname_prefix}-${var.app_name}-${var.env_name}-nic-ipconfig-name-${each.key}"
          static_ip                          = null        }
      ]
      tags = { vm_nic                        = "${var.hostname_prefix}-${var.app_name}-${var.env_name}-vmnic-${each.key}" }
  }
}
  
  #virtual_machines Definition
  virtual_machines = {
  for v in range(1, 2) : "${var.hostname_prefix}-${var.app_name}-${var.env_name}-${each.key}" => {
    name                                     = "${var.hostname_prefix}-${var.app_name}-${var.env_name}-${each.key}"
    ...
    #os_disk Definition
    os_disk = {
      name                                   = "${var.hostname_prefix}-${var.app_name}-${var.env_name}-osDisk-${each.key}"
    ...
    tags = { vm                              = "${var.hostname_prefix}-${var.app_name}-${var.env_name}-${each.key}" }
  }
}

#managed_data_disks Definition
managed_data_disks = {
  for d in range(1, lookup(var.vm_specs, each.key).managed_data_disks_count + 1) : "disk${d}" => {
      name                                   = "${var.hostname_prefix}-${var.app_name}-${var.env_name}-disk-${each.key}-disk${d}"
      vm_key                                 = "${var.hostname_prefix}-${var.app_name}-${var.env_name}-${each.key}"
      lun                                    = d - 1
      storage_account_type                   = lookup(var.vm_specs, each.key).use_default_disk_config ? var.disk_storage_types_oracle[d - 1] : var.disk_storage_types_observer[d - 1]
      disk_size_gb                           = lookup(var.vm_specs, each.key).use_default_disk_config ? var.disk_sizes_gb_oracle[d - 1] : var.disk_sizes_gb_observer[d - 1]
      caching                                = lookup(var.vm_specs, each.key).use_default_disk_config ? var.disk_caching_oracle[d - 1] : var.disk_caching_observer[d - 1]
      write_accelerator_enabled              = false
      create_option                          = null
      os_type                                = null
